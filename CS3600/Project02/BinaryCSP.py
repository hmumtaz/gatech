...
class NotSameDiagonalConstraint(BinaryConstraint):
    def isSatisfied(self, value1, value2):
        """Created using the createDict function to save time in computation"""
        variable1 = int(self.var1)
        variable2 = int(self.var2)
        difference = abs(variable1 - variable2)
        value1 = int(value1)
        value2 = int(value2)
        if (value1 + difference == value2) or (value2 + difference == value1):
            return False
        return True
    ...
    for var in csp.varDomains:
        if not assignment.isAssigned(var):
            if nextVar is None:
                ...
                nextVar = degreeHeuristic(csp, var, nextVar)
    return nextVar


def degreeHeuristic(csp, var1, var2):
    constraints = csp.binaryConstraints
    degree1 = 0
    degree2 = 0
    for constraint in constraints:
...
def leastConstrainingValuesHeuristic(assignment, csp, var):
    ...
    countsDict = {}
    for value in values:
        count = constraintCounter(value, csp, assignment, var)
        countsDict[value] = count
    values = list(sorted(countsDict, key=countsDict.get))
    return values


def constraintCounter(value, csp, assignment, var):
    counter = 0
    constraints = csp.binaryConstraints
    for constraint in constraints:
        if constraint.affects(var):
            neighbor = constraint.otherVariable(var)
            if value in assignment.varDomains[neighbor]:
                counter += 1
    return counter
        ...
        if constraint.affects(var):
            ...
            if value in domains[otherVar]:
                ...
                if len(domains[otherVar]) == 0:
                    while len(inferences) != 0:
                        infVar, infValue = inferences.pop()
                        domains[infVar].add(infValue)
                    return None
    return inferences
...
def revise(assignment, csp, var1, var2, constraint):
    ...
    if assignment.isAssigned(var1):
        val1 = assignment.assignedValues[var1]
        for val2 in valuesof2:
            if not constraint.isSatisfied(val1, val2):
                inferences.add((var2, val2))
                domains[var2].remove(val2)
                if len(domains[var2]) == 0:
                    while len(inferences) != 0:
                        infVar, infVal = inferences.pop()
...
def maintainArcConsistency(assignment, csp, var, value):
    ...
    for var in varQueue:
        for constraint in constraints:
            if constraint.affects(var):
                var2 = constraint.otherVariable(var)
                constraintInf = revise(assignment, csp, var, var2, constraint)
                if constraintInf is None:
                    while len(inferences) != 0:
                        infVar, infVal = inferences.pop()
                        domains[infVar].add(infVal)
                    return None
                elif len(constraintInf) > 0:
                    while len(constraintInf) != 0:
                        inferences.add(constraintInf.pop())
                    varQueue.append(var2)
    return inferences
