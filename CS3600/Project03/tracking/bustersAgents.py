...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for i in range(len(livingGhostPositionDistributions)):
            bestProb = 0
            bestPosition = None
            for position, prob in livingGhostPositionDistributions[i].items():
                if prob > bestProb:
                    bestProb = prob
                    bestPosition = position
            maybeGhostPosition.append((bestPosition, self.distancer.getDistance(bestPosition, pacmanPosition)))
        closest = float('inf') #infinity
        closestPosition = None
        for ghostPosition, distance in maybeGhostPosition:
            if distance < closest:
                closest = distance
                closestPosition = ghostPosition
        greedyDistance = float('inf')
        greedyActions = []
        for action in legal:
            newPosition = Actions.getSuccessor(pacmanPosition, action)
            newDistance = self.distancer.getDistance(closestPosition, newPosition)
            if newDistance < greedyDistance:
                greedyDistance = newDistance
                greedyActions = [action]
            elif newDistance == greedyDistance:
