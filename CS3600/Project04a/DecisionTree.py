...
def setEntropy(classCounts):
    ...
    for count in classCounts:
        pCount = float(count) / sum(classCounts)
        entropy -= pCount * log(pCount, 2)
    return entropy
...
def remainder(examples,attrName,attrValues,className):
    ...
    classCounts = classCountsDict.values()
    result = 0
    for value in attrValues:
        pertinentExamples = getPertinentExamples(examples, attrName, value)
        kClassCountsDict = getClassCounts(pertinentExamples, className)
        kClassCounts = kClassCountsDict.values()
...
def giniGain(examples,attrName,attrValues,className):
    ...
    for value in attrValues:
        pertinentExamples = getPertinentExamples(examples, attrName, value)
        classCountsDict = getClassCounts(pertinentExamples, className)
        classCounts = classCountsDict.values()
...
def makeSubtrees(remainingAttributes,examples,attributeValues,className,defaultLabel,setScoreFunc,gainFunc):
    ...
    for attribute in remainingAttributes:
        curBestGain = gainFunc(examples, best, attributeValues[best], className)
        attrGain = gainFunc(examples, attribute, attributeValues[attribute], className)
        if attrGain > curBestGain:
            best = attribute
    newRemainingAttributes = list(remainingAttributes)
    newRemainingAttributes.remove(best)
    root = Node(best)
    for attribute in attributeValues[best]:
        pertinentExamples = getPertinentExamples(examples, best, attribute)
        if len(pertinentExamples) == 0:
            child = Node(attribute)
            child.isleaf = True
            child.value = getMostCommonClass(examples, className)
            root.children[attribute] = child
        else:
            root.children[attribute] = makeSubtrees(newRemainingAttributes, pertinentExamples, attributeValues, className, defaultLabel, setScoreFunc, gainFunc)
    return root
