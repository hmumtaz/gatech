...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):                                       
        ...
        del inActs[0]
        return float(self.sigmoid(weightedSum))
    ...
    def sigmoidDeriv(self, value):
        ...
        """
        """YOUR CODE"""
        return float(exp(value)/((exp(value) + 1)**2))
    ...
    def sigmoidActivationDeriv(self, inActs):
        ...
        weightedSum = self.getWeightedSum(inActs)
        del inActs[0]
        return float(self.sigmoidDeriv(weightedSum))
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for weight in self.weights:
            old = weight
            weight = weight + (alpha * inActs[index] * delta)
...
class NeuralNet(object):                                    
    ...
    def feedForward(self, inActs):
        ...
        for curLayer in self.layers:
            pList = []
            for perceptron in curLayer:
                pList.append(perceptron.sigmoidActivation(input))
            input = pList
            retList.append(pList)
        return retList
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    testAccuracy = float(float(testCorrect) / float(testCorrect + testError))
